...
AForm::AForm(int signGrade, int executeGrade)
    : _name("default"), _isSigned(false), _signGrade(signGrade),
      _executeGrade(executeGrade) {
  std::cout << "create AForm default with sign " << signGrade << " & exec "
            << executeGrade << std::endl;
  this->_checkGrades(signGrade, executeGrade);
}

AForm::AForm(std::string const &name)
    : _name(name), _isSigned(false), _signGrade(150), _executeGrade(150) {
...
AForm::AForm(std::string const &name, int signGrade, int executeGrade)
    : _name(name), _isSigned(false), _signGrade(signGrade),
      _executeGrade(executeGrade) {
  std::cout << "create AForm " << this->_name << " with sign " << signGrade
            << " & exec " << executeGrade << std::endl;
  this->_checkGrades(signGrade, executeGrade);
}

AForm::~AForm() { std::cout << "destroy AForm " << this->_name << std::endl; }
...
void AForm::beSigned(Bureaucrat &bureaucrat) {
  if ((int)bureaucrat.getGrade() > this->_signGrade)
    ...
    std::cout << "\t" << bureaucrat.getName() << " couldn't sign "
              << this->getName() << " because it's already signed" << std::endl;
}
