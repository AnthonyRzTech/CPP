...
Form::Form(int signGrade, int executeGrade)
  ...
  std::cout << "create Form default with sign-grade" << signGrade
            << " and execution-grade " << executeGrade << std::endl;
...
Form::Form(std::string const &name, int signGrade, int executeGrade)
    : _name(name), _isSigned(false), _signGrade(signGrade),
      _executeGrade(executeGrade) {
  std::cout << "create Form " << this->_name << " with sign-grade " << signGrade
            << " and execution-grade " << executeGrade << std::endl;
  this->_checkGrades(signGrade, executeGrade);
}

Form::~Form() { std::cout << "destroy Form " << this->_name << std::endl; }
