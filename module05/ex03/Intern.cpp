...
Intern::Intern() { std::cout << "create Intern" << std::endl; }
Intern::Intern(Intern const &other) {
  std::cout << "create Intern copy" << std::endl;
  *this = other;
}
Intern::~Intern() { std::cout << "destroy Intern" << std::endl; }
...
AForm *Intern::makeForm(std::string const &form, std::string const &target) {
  AForm *(*formGenerator[])(std::string const &target) = {
      &makePresidential, &makeRobotomy, &makeShrubbery};
  std::string const formNames[] = {"presidential pardon", "robotomy request",
                                   "shrubbery creation"};
  for (int i = 0; i < 3; i++) {
    if (form == formNames[i]) {
  ...
  std::cout << "\t"
            << "Intern can't make " << form << std::endl;
  return (NULL);
}
