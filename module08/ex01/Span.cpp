...
const std::list<int> *Span::getList(void) const { return &_list; }
...
unsigned int Span::shortestSpan(void) const {
  ...
  for (std::list<int>::const_iterator it = _list.begin(); it != _list.end();
       ++it) {
    for (std::list<int>::const_iterator it2 = _list.begin(); it2 != _list.end();
         ++it2) {
      if (it == it2)
        continue;
