...
int PmergeMe::getVecSize() { return _vec.size(); }

int PmergeMe::getDeqSize() { return _deq.size(); }
...
bool PmergeMe::parseAndStoreNumbersInVec(int argc, char **argv) {
  ...
  if (_vec.size() < 2) {
    std::cerr << "Error:\nAt least 2 numbers are required as an argument."
              << std::endl;
    return false;
  }
  return true;
...
void PmergeMe::mergeInsertSortVec() {
  ...
  for (int i = 0; i < static_cast<int>(vecSecondElements.size()); i++) {
    ...
    std::vector<int>::iterator insertPos =
        std::lower_bound(vecFirstElements.begin(), vecFirstElements.end(), key);
...
void PmergeMe::mergeInsertSortDeq() {
  ...
  for (int i = 0; i < static_cast<int>(deqSecondElements.size()); i++) {
    ...
    std::deque<int>::iterator insertPos =
        std::lower_bound(deqFirstElements.begin(), deqFirstElements.end(), key);
