...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Time to process a range of " << pmergeMe.getVecSize()
            << " elements with std::vector: " << std::fixed
            << std::setprecision(5) << timeSortVec << " us" << std::endl;

  std::cout << "Time to process a range of " << pmergeMe.getDeqSize()
            << " elements with std::deque: " << std::fixed
            << std::setprecision(5) << timeSortDeq << " us" << std::endl;
