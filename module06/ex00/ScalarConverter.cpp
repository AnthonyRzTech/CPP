...
static bool isNanInf(std::string const &in) {
  return (in == "nan" || in == "nanf" || in == "+inf" || in == "-inf" ||
          in == "+inff" || in == "-inff" || in == "inf" || in == "inff");
}

static bool isSpecialChar(std::string const &in) {
  return (in.length() == 1 &&
          (in[0] == '+' || in[0] == '-' || in[0] == '.' || in[0] == 'f'));
}
...
void ScalarConverter::convert(std::string const &input) {
  ...
  case NAN_INF:
    ...
    break;
  case ERROR:
    std::cout << RED << "Error: Impossible to print or input not convertable"
              << std::endl;
    break;
  case CHAR:
...
void ScalarConverter::print(int type, std::string const &input, char c, int i,
                            float f, double d) {
  printChar(type, i, c);
